...
class App(Frame):
	def __init__(self, parent):
		...
		ttk.Button(mainframe, text="Classify from sample", command=lambda:classify(gender, filename, mainframe, False)).grid(column=3, row=5, sticky=W)
		ttk.Button(mainframe, text="Classify form microphone", command=lambda:record_and_classify(gender, mainframe, True)).grid(column=3, row=6, sticky=W)
		ttk.Label(mainframe, text= "Gender: ").grid(column=2, row=7, sticky=E)
		ttk.Label(mainframe, textvariable=gender).grid(column=3, row=7, sticky=E)
